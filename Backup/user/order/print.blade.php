...
<tr>
<td>{{ $items->id }}</td>
<td>{{mb_strlen($items->name,'utf-8') > 25 ? mb_substr($items->name,0,25,'utf-8').'...' : $items->name}}</td>
<td>
...
<b>{{ $langg->lang313 }}</b>:  <span id="color-bar" style="border: 10px solid {{$cart->color == "" ? "white" : '#'.$cart->color}};"></span>
...
<td>{{$order->currency_sign}}{{round($items->price * $order->currency_value,2)}}</td>
<td>{{$order->currency_sign}}{{round($cart->price * $order->currency_value,2)}}</td>
