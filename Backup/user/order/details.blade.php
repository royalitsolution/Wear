                    ...
                    <tbody>
                        <?php if(!empty($carts)){
                            ...
                            foreach($carts as $cart){
                                 $subtotal = 0;
                                $tax = 0;
                                $items = isset($cart->item) ? $cart->item : '';
                                ?>
                                <tr>
                                   <td>{{ $i++ }}</td>  <!-- $items->id --> 
                                   <td>
                                    <input type="hidden" value="{{ $cart->license }}">
                                    <?php if($items->user_id != 0){
                                        $user = App\Models\User::find($items->user_id);
                                        if(isset($user)){
                                            ?>
                                            <a target="_blank"
                                            href="{{ route('front.product', $items->slug) }}">{{mb_strlen($items->name,'utf-8') > 30 ? mb_substr($items->name,0,30,'utf-8').'...' : $items->name}}</a>
                                       ...
                                       <?php }}else{?>
                                        ...
                                        href="{{ route('front.product', $items->slug) }}">{{mb_strlen($items->name,'utf-8') > 30 ? mb_substr($items->name,0,30,'utf-8').'...' : $items->name}}</a>
                                    <?php }?>  
                                    <?php if($items->type != 'Physical'){
                                        if($order->payment_status == 'Completed'){
                                            if($items->file != null){
                                                ?>
                                                <a href="{{ route('user-order-download',['slug' => $order->order_number , 'id' => $items->id]) }}"
                               ...
                               <td>
                                <b>{{ $langg->lang311 }}</b>: {{$cart->qty}} <br>
                                @if(!empty($cart->size))
                                <b>{{ $langg->lang312 }}</b>: {{ $items->measure }}{{str_replace('-',' ',$cart->size)}} <br>
                                @endif
                                @if(!empty($cart->color))
                                <div class="d-flex mt-2">
                                    <b>{{ $langg->lang313 }}</b>:  <span id="color-bar" style="border: 10px solid {{$cart->color == "" ? "white" : '#'.$product['color']}};"></span>
                                </div>
                                @endif

                                @if(!empty($cart->keys))

                                @foreach( array_combine(explode(',', $cart->keys), explode(',', $cart->values))  as $key => $value)
                            ...
                            <td>
                                {{$order->currency_sign}}{{round($items->price * $order->currency_value,2)}}
                            </td>
                            <td>{{$order->currency_sign}}{{round($cart->price * $order->currency_value,2)}}
                            </td>
