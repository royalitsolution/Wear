...
<label for="ship-diff-address">{{ $langg->lang160 }}</label>
 ...
 Shipping Details  :
	...
	<div class="address-body">
			<a href="#" id="add_address" class="add-address hover-btn" data-toggle="modal" data-target="#address_model">Add New Address</a>
			<div id="address_list">
...
<?php /* <input type="hidden" name="coupon_code" id="coupon_code" value="{{ Session::has('coupon_code') ? Session::get('coupon_code') : '' }}">
...
<div class="col-lg-6">
<input class="form-control ship_input" type="radio" name="address_type"
id="address_type" value="home">Home
<input class="form-control ship_input" type="radio" name="address_type"
id="address_type" value="office">Office
<input class="form-control ship_input" type="radio" name="address_type"
id="address_type" value="others">Others
</div>
</div>
      ...
      <div class="modal-footer">
        ...
        <button type="button" id="add_new_address" class="btn btn-primary">Save</button>
      </div>
    </div>
  </div>
</div>
...
<script type="text/javascript">
	...
	function get_address_list(){
		var user_id = '<?php echo isset(Auth::user()->id) ? Auth::user()->id : ''?>';
		if(user_id == ''){
			return false;
...
<script type="text/javascript">
	function delete_confirm(){
		var su = confirm('Are you Want to delete this Address');
		if(su){
			var _token = '{{ csrf_token() }}';
...
<script type="text/javascript">
	$('#add_new_address').click(function(e) {
   		var shippingFull_name = $('#shippingFull_name').val();
  		var shipingPhone_number = $('#shipingPhone_number').val();
  		var shipping_address = $('#shipping_address').val();
  		var shipping_country = $('#shipping_country').val();
  		var shipping_city = $('#shipping_city').val();
      var address_type = $('input[name="address_type"]:checked').val();
  		var shippingPostal_code = $('#shippingPostal_code').val();

  		var _token = '{{ csrf_token() }}';

            $.ajax({
                url: "{{ route('add_address_list') }}",
                type: "POST",
                data: {shippingFull_name:shippingFull_name,shipingPhone_number:shipingPhone_number,shipping_address:shipping_address,shipping_country:shipping_country,shipping_city:shipping_city,shippingPostal_code:shippingPostal_code,address_type:address_type},
                dataType:"JSON",
                headers:{'X-CSRF-TOKEN': _token},
                cache: false,
             	success: function(resp){
...
<script type="text/javascript">
  $('#edit').on('click', function() {
    var id = $(this).data('id'); // the rest is just the same
    //saveNote(id);
    $('#address_model').modal('toggle'); // this is to close the modal after clicking the modal button
  })
